...
def extract_images_from_excel(file, sheet_name):
    ...
    try:
        ...
        print(f"Nombre total d'images dans la feuille : {len(worksheet.images)}")

        for idx, image in enumerate(worksheet.images):
            try:
                row_index, image_bytes = _extract_image_info(image, idx, temp_dir)
